...
def cond_fund_science(ship):
    # return has_enough_money(ship, FUND_SCIENCE_COST)
    return True
...
def act_sell_artifact(ship):
    ...
    ship.ruler.money += MONEY_PER_ARTIFACT
...
def act_fund_science(ship):
    ...
    ship.ruler.technology.science[1] += 5

def act_schmooze(ship):
    ship.ruler.game.diplomacy.gain_leverage(ship.ruler.id, ship.star.ruler.id,
                                            ship.ruler.technology.get_schmooze_power())
    ship.ruler.milestone_progress[3] += ship.ruler.technology.get_schmooze_power()
...
def act_raid_minerals(ship):
    ...
    ship.ruler.game.diplomacy.lose_leverage(ship.ruler.id, ship.star.ruler.id, 1)
...
def act_plunder(ship):
    ...
    ship.ruler.money += 1000
    ship.ruler.milestone_progress[4] += 1000 / MONEY_MILESTONE_VALUE
    ship.ruler.milestone_progress[0] += 10
    ship.ruler.game.diplomacy.lose_leverage(ship.ruler.id, ship.star.ruler.id, 5)
...
class Action:
    ...
    def __init__(self, condition_fn, action_fn, rate_fn, money_cost_fn=lambda t: 0, repeat=False):
        self.condition_fn = condition_fn
        self.action_fn = action_fn
        self.rate_fn = rate_fn
        self.money_cost_fn = money_cost_fn
        self.repeat = repeat

    def perform(self, ship, time):
        if self.condition_fn(ship):
            if ship.try_reserve_money(self.money_cost_fn(ship.ruler.technology)):
                ship.action_progress += time * self.rate_fn(ship.ruler.technology)
                if ship.action_progress >= 1.0:
                    self.action_fn(ship)
                    ship.reserved_money = 0
                    if not self.repeat:
                        ship.set_action(0)
                    else:
                        ship.action_progress = 0.0
            else:
                ship.ruler.log_message("Not enough money to perform the action.")
                ship.set_action(0)
        else:
            ship.set_action(0)
...
# 2 - A function that takes a TechnologyTree object and returns the rate, per minute, of the
#     action being completed. Many might not use the TechnologyTree passed, but it will be passed.
# 3 - A function that takes a TechnologyTree object and returns the money cost of the actionn
# 4 - A boolean value that says whether or not the action automatically repeats after completion
SHIP_ACTIONS = [
    (lambda a: False, lambda a: None, lambda t: 0, lambda t: 0, False),
    (lambda a: False, lambda a: None, lambda t: 0, lambda t: 0, False),
    (cond_build_city, act_build_city, lambda t: t.get_city_build_rate(), lambda t: 0, False),
    (cond_develop_colony, act_develop_colony, lambda t: t.get_development_rate(), lambda t: 0, False),
    (cond_establish_colony, act_establish_colony, lambda t: t.get_colonise_rate(), lambda t: 0, False),
