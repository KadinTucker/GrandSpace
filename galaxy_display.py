...
class GalaxyDisplay(pane.Pane):
    ...
    def sketch_primary_surface(self):
        for i in range(len(self.game.galaxy.stars)):
            ...
            pygame.draw.circle(self.layers[1], star_color, self.project_coordinate(s.location), int(self.view_scale * GALAXY_STAR_RADIUS))
            ...
            for p in range(numPlanets):
                ...
                pygame.draw.circle(self.layers[1], galaxy.MINERAL_COLORS[s.planets[p].mineral], self.project_coordinate(planetCenter), int(self.view_scale * GALAXY_PLANET_RADIUS))
    ...
    def sketch_player_surface(self):
        for i in range(len(self.game.galaxy.stars)):
            ...
            if self.player.explored_stars[i]:
                if s.ruler != None:
                    pygame.draw.circle(self.layers[0], s.ruler.color, self.project_coordinate(s.location), int(COLONY_HALO_MODIFIER * self.view_scale * GALAXY_STAR_RADIUS))
                    if s == s.ruler.homeworld.star:
                        draw_five_point_star(self.layers[0], s.ruler.color, self.project_coordinate(s.location), int(CAPITAL_STAR_MINOR_MODIFIER * self.view_scale * GALAXY_STAR_RADIUS), int(CAPITAL_STAR_MAJOR_MODIFIER * self.view_scale * GALAXY_STAR_RADIUS))
        for p in self.game.players:
            for s in p.ruled_stars:
                if self.player.explored_stars[s.id] and s.connected_star != None:
                    pygame.draw.line(self.layers[0], p.color, self.project_coordinate(s.location), self.project_coordinate(s.connected_star.location), int(self.view_scale * EMPIRE_CONTROL_LINE_WIDTH))
    ...
    def handle_event(self, event, mouse_pos, active_player):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == pygame.BUTTON_RIGHT:
                ...
                if active_player.selected_ship.destination_star != None:
